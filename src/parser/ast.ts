...
export interface Module {
  ...
  envSpecs?: Array<{ key: string; constraints: string }>; // Native-Env-Shield: @require_env(key: "...", ...)
  secureHashSpecs?: Array<{ field: string; algo: string; rounds?: number; isFunction?: boolean }>; // Native-Secure-Hasher: @secure_hash(algo: .bcrypt, rounds: 12)
  stateAtoms?: Array<StateAtomConfig>;  // Native-State-Atom v2.35: @state 어노테이션 (Zustand 대체)
  sessionContexts?: Array<SessionContextConfig>;  // Native-Session-Vault v2.37: @session_context(...) 커널 SHM 기반 세션
  immutableStructs?: Array<{ name: string; fields: Array<{ name: string; type: string }> }>;  // v2.39: @immutable(...) immer 대체
}

/**
 * Native-State-Atom v2.35: @state(...) 어노테이션 설정
 *
 * Zustand/Redux 대체 - FreeLang v2 네이티브 반응형 상태 관리
 *
 * 예시:
 *   @state
 *   struct WashParkStore {
 *     count: int = 0,
 *     status: string = "idle"
 *   }
 */
export interface StateAtomConfig {
  name: string;                    // 상태 원자명 (예: "WashParkStore")
  fields: Array<{
    name: string;                  // 필드명 (예: "count")
    type: string;                  // 필드 타입 (예: "int", "string")
    initialValue?: any;            // 초기값 (선택)
  }>;
  isDirtyTracking?: boolean;       // Dirty-check 비트 플래그 추적
  observerCallbacks?: Array<{
    field: string;                 // 감시할 필드명
    callback: string;              // 콜백 함수명
  }>;
...
/**
 * Native-Session-Vault v2.37: @session_context(...) 어노테이션 설정
 *
 * 커널 공유 메모리(SHM) 기반 고속 세션 관리
 * express-session 완전 대체
 *
 * 예시:
 *   @session_context(store: .shm, encrypt: true)
 *   struct UserSession {
 *     user_id: u64,
 *     role: string[16],
...
export interface SessionContextConfig {
  name: string;                      // 세션 구조체명 (예: "UserSession")
  store: 'shm' | 'memory' | 'file';  // 저장소 타입 (SHM 권장)
  encrypt?: boolean;                 // 암호화 여부 (기본: true)
  ttl?: number;                      // TTL (초 단위, 기본: 3600)
  fields: Array<{
    name: string;                    // 필드명
    type: string;                    // 필드 타입 (u64, string[N], timestamp 등)
    offset?: number;                 // 메모리 오프셋 (자동 계산)
...
/**
 * @mongo_store 어노테이션 설정
 * Native-Mongo-Vault v2.42용 MongoDB 저장소 정의
 *
 * 사용 예:
 * @mongo_store(uri: "mongodb://localhost:27017", collection: "sessions")
...
export interface MongoStoreConfig {
  name: string;  // 구조체명
  uri: string;   // MongoDB 연결 URI
  collection: string;  // 컬렉션명
  maxConnections?: number;  // 연결 풀 크기
  poolTimeout?: number;  // 풀 타임아웃 (ms)
  fields: Array<{
    name: string;
    type: string;  // FreeLang 타입: u64, i64, f64, string, json, timestamp, bytes, etc
    offset?: number;  // 메모리 오프셋 (자동 계산)
    size?: number;    // 필드 크기 (자동 계산)
  }>;
  indexes?: Array<{
    fields: string[];
    unique?: boolean;
...
/**
 * @session_context 어노테이션 설정
 * Native-Session-Vault v2.37용 세션 저장소 정의
 */
export interface SessionContextConfig {
  name: string;
  fields: Array<{
